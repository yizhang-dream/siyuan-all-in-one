...
## A/B 界面布局对比（SiYuan 真机 vs NotebookLM 公开文档）

基于本插件在真实 SiYuan v3.6.5 中的截图证据（`docs/siyuan-all-in-one-screenshot.png`，737KB，15 插件共存）与 NotebookLM 2025-2026 公开功能说明的界面布局对比：

```
┌─────────────────────────────────┐  ┌─────────────────────────────────┐
│  SiYuan 3.6.5 + 本插件          │  │  Google NotebookLM (2025-2026)  │
├─────────────────────────────────┤  ├─────────────────────────────────┤
│ ┌──────┬──────────────────────┐ │  │ ┌──────────┬──────────────────┐ │
│ │侧栏  │ 复习面板              │ │  │ │来源列表  │ 聊天/问答区       │ │
│ │导航  │ 牌组选择 + 每日上限   │ │  │ │(PDF/URL) │ 内联引用跳转      │ │
│ │10个  │ 问题 + 提示（同页）   │ │  │ │          │                  │ │
│ │图标  │ LaTeX $$公式$$ 渲染   │ │  │ ├──────────┼──────────────────┤ │
│ │标签  │ 答案 + 评分按钮       │ │  │ │ Studio   │ 脑图(自动生成)   │ │
│ │      │                      │ │  │ │ 面板     │ 闪卡(无FSRS)     │ │
│ ├──────┼──────────────────────┤ │  │ │          │ 音频播客          │ │
│ │      │ 图谱生成面板          │ │  │ │          │ 视频概述          │ │
│ │      │ 手动/OpenNB/混合来源  │ │  │ │          │                  │ │
...
**关键差异总结：**
- **本插件独有：** ConceptNode 中间层、候选编辑保持关联、缺卡检测、FSRS+Final Drill、离线模式、多 Provider、图片遮挡编辑器、Unstructured 内置分区器
- **NotebookLM 独有：** Audio Overview（播客）、Video Overview、多模态来源（PDF/视频/YouTube）、Google Drive 同步
- **持平：** 来源引用、公式渲染、闪卡生成
