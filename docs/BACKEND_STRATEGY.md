...
- 已完成：Diagnostics 可观测 provider 结构化输出策略。
- 继续：Diagnostics 里显示 OpenNotebook schema/version、text/vector 健康状态。

### P1：SourceHub 抽象 ✅

- 已完成：`src/libs/sources/source-hub.ts` 统一 `collectPipelineSources()`。
- 已完成：手动、OpenNotebook、SiYuan、本地文件四种来源适配器。
- 已完成：Concepts.svelte 已迁移到 SourceHub，不再直接拼接来源。
- 已完成：合约测试 `test:source-hub` 覆盖混合来源和离线模式。

### P2：轻量本地文件解析 ✅ (文本 + HTML)

- 已完成：`src/libs/sources/local-file-adapters.ts` 支持 `.txt/.md/.html/.htm`。
- 已完成：HTML 保守去标签和实体解码，长文本切块为 `file` 类型 `PipelineSource`。
- 已完成：Concepts 面板”手动 + 文档”模式允许 SiYuan 文档 + 本地文件混合来源。
- 未完成：浏览器端 PDF/docx/pptx 解析。
- 未完成：Unstructured helper 集成（保留为 P3 可选 worker）。

### P3：可选 Unstructured helper ✅ (内置分区器)

- 已完成：`src/libs/sources/unstructured-partitioner.ts` — 纯 JS Unstructured 兼容分区器。
- 支持 Markdown/HTML/纯文本 → Title/NarrativeText/ListItem/Table/CodeBlock 元素。
- `textToUnstructuredPipelineSources()` 一站式转为 PipelineSource[]。
- 已接入 SourceHub `unstructuredText` 选项。
- 测试：`test:unstructured` 覆盖 Markdown 分区和元素类型识别。
- 不打包 Python 依赖，不默认启动外部服务。

### P4：OpenNotebook 降级策略 ✅

- 已完成：OpenNotebook 不可用时，生成区自动隐藏 OpenNotebook/混合来源按钮，默认”手动文本”或”手动 + 文档”。
- 已完成：`hasOpenNotebook` 响应式守卫，端点消失时自动切回 manual 模式。
- 已完成：vector 搜索失败只降级本次搜索，不阻断 selected source/note 详情读取。
- 已完成：搜索结果无正文时优先按 source id 补 `getSource` 全文。
